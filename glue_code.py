...
logging.getLogger('pcse').propagate = False
...
def get_corners_lat_long(fname):
    ...
    for loc in locs:
        logging.info(f"Image corners: ({loc[0]:g}, {loc[1]:g})")
    return locs
...
def find_lai_files(file_path, tile):
    ...
    logging.info(f"Found {len(doys):d} files")
    logging.info(f"First date is {dates[0].strftime('%Y-%b-%d'):s}")
    logging.info(f"Last date is {dates[-1].strftime('%Y-%b-%d'):s}")
    return files, dates, doys, corner_locs


def run_wofost_ensembles(
    ...
    for corner in corner_locs:
        ...
        enwofost.ensemble_wofost(
            lon=lng,
            lat=lat,
            start=start_date,
            end=None,
            en_size=en_size,
            prior_file=parameter_priors,
            weather_type="ERA",
            weather_path="/home/ucfahm0/NC/",
    ...
    for ens_file in ensembles:
        ...
        this_time = np.array(
            [
                list(
                    map(
                        lambda x: datetime.date.strftime(x, "%Y%j"), ens["day"]
                    )
                )
                for ens in f
            ]
...
def match_ensembles_to_lai(
    files, dates, doys, ens_days, ens_lai, ens_yield, threshold=3.5
):
    matched_files = []
    matched_ensemble = []
    matched_ens_times = []
    ...
    for (
        i,
        (file_info, this_X, this_Y, nx_valid, ny_valid, data_chunks),
    ) in enumerate(extract_chunks([str(f) for f in matched_files])):
        logging.info(f"Chunk #{i+1:d}")
        data_chunks = np.array(data_chunks)
        mask = np.logical_and(data_chunks >= 0.0, data_chunks <= 10.0)
        data_chunks[~mask] = np.nan

        D = (
            data_chunks[:, :, :, None]
            - np.array(matched_ens_lai)[:, None, None, :]
        )
...
if __name__ == "__main__":
    ...
    ens_days, ens_lai, ens_yield = run_wofost_ensembles(
        corner_locs, en_size=50, start_date=datetime.date(2017, 10, 12)
    )

    assimilated_yield_mean, assimilated_yield_std = match_ensembles_to_lai(
        files, dates, doys, ens_days, ens_lai, ens_yield
    )
    np.savez_compressed(
        "assimilated_yield_mean.npz",
        assimilated_yield_mean=assimilated_yield_mean,
    )

    np.savez_compressed(
        "assimilated_yield_std.npz",
